...
<body>
    ...
    <?php
        ...
        $DatosDeLaConsulta = $BaseDeDatos->consultar("SELECT * FROM `cotizaciones` WHERE `idEstado` = 36");

        try{
            foreach($DatosDeLaConsulta as $RowBorrador){
                $ConsultaDeProductos = $BaseDeDatos->consultar("SELECT * FROM `cuerpocotizacion` WHERE `idCotizacion` = ".$RowBorrador['id']);

                $BaseDeDatos->ejecutar("DELETE FROM `cuerpocotizacion` WHERE `idCotizacion` = ".$RowBorrador['id']);
                $BaseDeDatos->ejecutar("DELETE FROM `cotizaciones` WHERE `id` = ".$RowBorrador['id']);
            }
            header('Location: ../Ventas/NuevaCotizacion/');
        }catch(Exception $e){
            echo $e->getMessage();
        }
    ?>
