...
<body>
    ...
    <?php
        ...
        $ListaDeProveedoresEnBorrador = $BaseDeDatos->consultar("SELECT * FROM `proveedores` WHERE `idEstado` = 9");

        try{
            foreach($ListaDeProveedoresEnBorrador as $DatosDeProveedorEnBorrador){
                $ProveedorEnBorrador = new proveedor($DatosDeProveedorEnBorrador['rif']);
                $ProveedorEnBorrador->Eliminar();
                header('Location: ../Proveedores/NuevoProveedor.php');
            }
        }catch(Exception $e){
            echo $e->getMessage();
        }
