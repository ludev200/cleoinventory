...
async function deleteEntity(id){
    try{
        response = await fetch(`http://${ipserver}/CleoInventory/API/entityfunctions.php?idModulo=7&idEntity=${id}&method=delete`);
        ...
        if(response.status == 200){
            ...
            if(petition.status == 200){
                Toast.fire({
                    icon: 'success',
                    title: 'Se ha eliminado el proveedor #'+id
                })
                
                await sleep(2500);
                window.location.href = `http://${ipserver}/CleoInventory/Proveedores`;
            }else{
                Toast.fire({
                    icon: 'error',
